...
t_strlist	*get_expansions_list(char *to_exp, int *indexes, t_strlist *env)
{
	...
	while (1)
	{
		...
		if (indexes[y] == 0)
			break;
		var = get_env_var_val_from_slice(&to_exp[indexes[i]], &to_exp[indexes[y]], env->envp);
...
static int (*func_table[])(char *, int*, int**) = {
	&normal_mode,
	&var_start_mode,
	&var_mode,
};

//Go to next dollar that is not a single dollar
...
int	*register_expansion(char* to_exp)
{
	...
	*indexes = 0;
	...
	box = indexes;
	while (state != DONE)
		state = func_table[state](to_exp, &i, &box);
	log_index(i, &box);
	log_index(0, &box);
	return (indexes);
}
